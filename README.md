...
当前仓库先开放 README 和效果展示图，插件代码会在整理完成后再开放。代码开放后，推荐直接把下面这段话复制给 Claude Code、Codex 或其他能操作本地文件的 Agent，让它全程帮你安装：

### 让 Claude / Codex 帮你安装（推荐）
...
如果你对这个方向感兴趣，欢迎先点个 **Star**。Star 和 Issue 反馈会决定我优先补哪些主题、封面风格和使用教程。
...
可以。代码开放后，README 上方的提示词可以直接复制给 Claude Code、Codex 或其他本地 Agent，让 Agent 帮你确认 Vault、安装到 `.obsidian/plugins/`，并引导启用插件。
